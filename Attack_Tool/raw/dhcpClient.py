...
def getMacInBytes():
    mac = str(hex(get_mac()))
    # mac = sys.arg[1] # spoofed mac address
    mac = mac[2:]
    while len(mac) < 12 :
        mac = '0' + mac
    macb = b''
...
if __name__ == '__main__':
    ...
    dhcpSrv.sendto(discoverPacket.buildPacket(), ('<broadcast>', 67))
    ...
    dhcpSrv.close()   #we close the socket
    ...
    # try:
    #     dhcps.bind(('192.168.5.100', 68))    #we want to send from port 68
